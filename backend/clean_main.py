...
def call_meme_agent_api(topic: str, style: str, tone: str, template_choice: str):
    ...
    try:
        ...
        if result.returncode != 0:
            # If there's an error but contains valid JSON, return that
            try:
    ...
    try:
        ...
        if not meme_intent:
            return {
                ...
                "data": {
                    "reply": "Hey! 👋 Kese ho? Agar memes banwane hain toh topic batao!",
                    "memes": [],
                    "meme_intent": False
                }
            }
        ...
        # CRITICAL: Check if result is already structured JSON
        if isinstance(result, dict) and "memes" in result:
            ...
            if not reply or "meme" in reply.lower():
                reply = f"Bhai {request.message} ke liye ye 3 special memes dekho! 😂"
            ...
            return {
                ...
                "data": {
                    ...
                    "template": request.template,
                    "raw_output": raw_output,
                    "reply": f"{request.message} par memes hazir hain! 😂",
                    "memes": memes,  # EXACTLY 3
                    "meme_intent": True
                }
            }
